                        ...
                        /* holiday
{
                    ...
                    for (const deviceKey in res.data) {
                        ...
                        for (const stats of res.data[deviceKey].data) {
                            ...
                            for (const resolution of resolutions) {
                                ...
                                await this.requestClient({
                                    method: "get",
                                    url: `https://api.vaillant-group.com/service-connected-control/end-user-app-api/v1/emf/v2/${
                                        id
                                    }/devices/${res.data[deviceKey].device_uuid}/buckets?resolution=${resolution}&operationMode=${
                                        stats.operation_mode
                                    }&energyType=${stats.value_type}&startDate=${fromDate}&endDate=${toDate}`,
                                    headers: {
                                        Authorization: `Bearer ${this.session.access_token}`,
                                    },
                                })
    ...
    async updateMyvEfficiency() {
        for (const device of this.deviceArray) {
            ...
            const url = `https://api.vaillant-group.com/service-connected-control/end-user-app-api/v1/emf/v2/${
                id
            }/currentSystemWithEfficiency?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(
                endDate,
        ...
        return new Promise((resolve, reject) => {
            this.mmClient({
                ...
                .then(async resp => {
                    ...
                    try {
                        // eslint-disable-next-line @typescript-eslint/no-this-alias -- traverse callbacks rebind `this`, so keep a reference to the adapter
                        const adapter = this;
            ...
            this.mmClient({
                ...
                .catch(err => {
                    ...
                    if ((resp && resp.status === 401) || JSON.stringify(body) === "NOT_AUTHORIZED") {
                        ...
                        this.log.error(err);
                        resp && this.log.error(resp.status);
    ...
    return new Promise(async (resolve, reject) => {
            ...
            if (id.indexOf("configuration") !== -1) {
                ...
                if (idPath[1] === "rooms") {
                    ...
                    url = `https://smart.vaillant.com/mobile/api/v4/facilities/${this.serialNr}/rbr/v1/rooms/${roomId}/configuration/${
                        action
                    }`;
                }
                body[action] = val;
            ...
            if (!state.ack) {
                if (this.config.myv) {
                    ...
                    if (command === "eebusEnabled") {
                        ...
                        url = `https://api.vaillant-group.com/service-connected-control/end-user-app-api/v1/ship/${deviceId}/self/spine`;
                    }
                    if (command === "holiday") {
                        ...
                        if (identifier !== "tli") {
                            ...
                            url = `https://api.vaillant-group.com/service-connected-control/${identifier}/v1/systems/${deviceId}/holiday`;
                        }
                    }
                    ...
                    if (command === "ventilationOperationMode" || command === "ventilationFanStage") {
                        ...
                        url = `https://api.vaillant-group.com/service-connected-control/end-user-app-api/v1/systems/${
                            deviceId
                        }/tli/ventilation/${ventilationIndex}/${postfix}`;
                        if (identifier !== "tli") {
                            url = `https://api.vaillant-group.com/service-connected-control/${identifier}/v1/systems/${
                                deviceId
                    ...
                    if (command === "customCommand") {
                        try {
                            ...
                            url = `https://api.vaillant-group.com/service-connected-control/end-user-app-api/v1/systems/${deviceId}/tli/${
                                parsedCommand.url
                            }`;
                            if (identifier !== "tli") {
                                url = `https://api.vaillant-group.com/service-connected-control/${identifier}/v1/systems/${deviceId}/${
                                    parsedCommand.url
                                }`;
                            }
